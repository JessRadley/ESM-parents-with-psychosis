...
plot(gdat, pch=19, cex=.5)
plot(gdat, pch=19, cex=.5, subset = gdat$id %in% c(11,32,3,34,22,28,6,17,4,2), xlab="Observation number", ylab="Psychosis")  #this is the one i used for thesis
...
res2<-lmer(parent_act_stress ~ obs + lpsychosis + mpsychosis + (1|id), data = dat, REML = F)
lrtest(res1, res2)
#including mean levels of psychosis or negative affect does not make model superior

#Hypothesis 4 - add age and gender level 2 variables
res3<-lmer(parent_act_stress ~ obs + lpsychosis + mpsychosis  + age + gender + (1|id), data = dat, REML = F)
summary(res3)
confint(res3)
lrtest(res2, res3) #age and gender sig improve model
tab_model(res3, show.se = T, df.method = "satterthwaite",file = "Hyp4_activity.doc")

res4<-lmer(parent_act_stress ~ obs + lpsychosis + mpsychosis + age + gender + soc_support + child_behav + coping + (1|id), data = dat, REML = F)
lrtest(res3, res4) #measured variables do not improve model

#####Hypothesis 2 - improvement of adding time-invariant variables to the model
res5<-lmer(parent_act_stress ~ obs + lpsychosis + mpsychosis + age + gender + soc_support + child_behav + coping + parent_selfeffic + (1|id), data = dat, REML = F)
summary(res5)
confint(res5)
AIC(res4)
AIC(res5)
tab_model(res5, show.se = T, df.method = "satterthwaite",file = "Hyp4_activity_covariates.doc")
#cannot do likelihood ratio test with res7 because it deletes 3 participants. However, AIC is lower but this might be because of the deleted participants?

####moderation
#social support
res6<-lmer(parent_act_stress ~ obs + lpsychosis*soc_support+ mpsychosis  + age + gender +  + child_behav + coping + parent_selfeffic + (1|id), data = dat, REML = F)
summary(res6)
lrtest(res5, res6) #social support moderation is non-sig

#child behaviour
res7<-lmer(parent_act_stress ~ obs + lpsychosis*child_behav + mpsychosis + age + gender + soc_support + coping + parent_selfeffic + (1|id), data = dat, REML = F)
summary(res7)
confint(res7)
lrtest(res5, res7) #child behaviour is just non-sig

#coping
res8<-lmer(parent_act_stress ~ obs + lpsychosis*coping + mpsychosis + age + gender + soc_support + child_behav + parent_selfeffic + (1|id), data = dat, REML = F)
summary(res8)
lrtest(res5, res8) #coping moderation is just non-sig

#parenting self-efficacy
res9<-lmer(parent_act_stress ~ obs + lpsychosis + coping + mpsychosis + age + gender + soc_support + child_behav + lpsychosis*parent_selfeffic + (1|id), data = dat, REML = F)
summary(res9)
lrtest(res5, res9) #parenting self-efficacy is non-sig

#gender
res10<-lmer(parent_act_stress ~ obs + lpsychosis*gender + coping + mpsychosis + age + gender + soc_support + child_behav + parent_selfeffic + (1|id), data = dat, REML = F)
summary(res10)
confint(res10)
lrtest(res5, res10) #gender moderation is sig
tab_model(res10, show.se = T, df.method = "satterthwaite",file = "Hyp4_activity_gender_mod.doc")

#all moderators
res11<-lmer(parent_act_stress ~ obs + lpsychosis*gender + lpsychosis*parent_selfeffic + lpsychosis*coping + lpsychosis*child_behav  + lpsychosis*soc_support +  + mpsychosis +  + age + parent_selfeffic + (1|id), data = dat, REML = F)
summary(res11)
lrtest(res5, res11)
tab_model(res11, show.se = T, df.method = "satterthwaite",file = "Hyp4_activity_allmod.doc")
...
res5<-lmer(parent_act_stress ~ obs + lpsychosis + mpsychosis + age + gender + (1|id), data = dat, REML = F)
res7<-lmer(parent_act_stress ~ obs + lpsychosis + mpsychosis + age + gender + soc_support + child_behav + coping + parent_selfeffic + (1|id), data = dat, REML = F)
res13<-lmer(parent_act_stress ~ obs + lpsychosis*gender + coping + mpsychosis + age + gender + soc_support + child_behav + parent_selfeffic + (1|id), data = dat, REML = F)

models <- with(imp, lmer(act_stress ~ obs + lpsychosis + mpsychosis + age + gender + (1|id), REML = F, control = lmerControl(optimizer = "Nelder_Mead")))
res5m<-summary(pool(models), conf.int = TRUE)
ft<-flextable(res5m)%>% colformat_double(digits = 3)
save_as_docx(ft, path = "mHyp4_act.docx")

models <- with(imp, lmer(act_stress ~ obs + lpsychosis  + mpsychosis  + age + gender + soc_support + child_behav + coping + parent_selfeffic + (1|id), REML = F, control = lmerControl(optimizer = "Nelder_Mead")))
res7m<-summary(pool(models), conf.int = TRUE)
ft<-flextable(res7m)%>% colformat_double(digits = 3)
save_as_docx(ft, path = "mHyp4_act_covariates.docx")

models <- with(imp, lmer(act_stress ~ obs + lpsychosis*gender + coping + mpsychosis + age + gender + soc_support + child_behav + parent_selfeffic + (1|id), REML = F, control = lmerControl(optimizer = "Nelder_Mead")))
...
res5e<-lmer(parent_act_stress ~ obs + psychosis + lpsychosis + lparent_act_stress + mpsychosis + age + gender + (1|id), data = dat, REML = F)
summary(res5e)
